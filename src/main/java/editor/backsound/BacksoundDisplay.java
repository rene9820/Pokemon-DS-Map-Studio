    ...
    private void formMouseDragged(MouseEvent e) {
        if (backsoundHandler.getSoundplates().size() > 0) {
            if (dragging && isCursorInsiePanel(e)) {
                ...
                switch (partSelected) {
                    case CENTER -> updatePlate(p, lastPlateX + deltaX, lastPlateY + deltaY, p.width, p.height);
                    case CORNER_SE -> updatePlate(p, p.x, p.y, lastPlateWidth + deltaX, lastPlateHeight + deltaY);
                    case CORNER_NW -> updatePlate(p, lastPlateX + deltaX, lastPlateY + deltaY, lastPlateWidth - deltaX, lastPlateHeight - deltaY);
                    case CORNER_NE -> updatePlate(p, p.x, lastPlateY + deltaY, lastPlateWidth + deltaX, lastPlateHeight - deltaY);
                    case CORNER_SW -> updatePlate(p, lastPlateX + deltaX, p.y, lastPlateWidth - deltaX, lastPlateHeight + deltaY);
                    case BORDER_N -> updatePlate(p, p.x, lastPlateY + deltaY, p.width, lastPlateHeight - deltaY);
                    case BORDER_S -> updatePlate(p, p.x, p.y, p.width, lastPlateHeight + deltaY);
                    case BORDER_W -> updatePlate(p, lastPlateX + deltaX, p.y, lastPlateWidth - deltaX, p.height);
                    case BORDER_E -> updatePlate(p, p.x, p.y, lastPlateWidth + deltaX, p.height);
                }
                repaint();
            }
        }
